...
def login_view(request):
    ...
    return render(request, "ordexa/login.html")
...
def register_view(request):
    ...
    return render(request, "ordexa/register.html")
...
def orden_list(request):
    ...
    return render(request, "ordexa/orden_list.html", {"ordenes": ordenes})
...
def orden_create(request):
    ...
    return render(request, 'ordexa/orden_form.html', {
        'form': form,
        'formset': formset,
...
def orden_pdf(request, pk):
    ...
    response['Content-Disposition'] = f'attachment; filename=cotizacion_{orden.id}.pdf'
    ...
    logo_path = os.path.join(settings.BASE_DIR, "ORDEXA", "static", "images", "logoo.png")
    ...
    descuento = subtotal * Decimal("0.05")  # 5% de descuento
    total = subtotal - descuento  # se resta del subtotal

    totales = [
        [Paragraph("Valor total:", styles["Normal"]),
         Paragraph(f"${formato_numero(subtotal)}", styles["Normal"])],
        [Paragraph("Descuento transferencia electrónica (5%):", styles["Normal"]),
